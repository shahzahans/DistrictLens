...
The final embedded proposed-plan HTML maps are `proposed_california_plan.html` and `Louisian_Proposed_plan.html`. Congressional district view shows these maps inside the app with a short title and description, then displays the fairness metric table directly below each map. In the Louisiana section, the original `LA proposed reock map.html` appears first for comparison, followed by the final map titled `Final Proposed Louisiana Plan`.
